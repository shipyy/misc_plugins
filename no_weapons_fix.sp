...
public void OnPluginStart()
{
    ...
    cvSkipBots = CreateConVar("no_weapon_fix_skip_bots", "0", "Set to 1 to give bots a weapon, 0 otherwise");
    g_bSkipBots = GetConVarBool(cvSkipBots);
    HookConVarChange(cvSkipBots, OnSettingChanged);
...
{
    if(g_bSkipBots && IsFakeClient(client))
        return;
...
public int OnSettingChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
    ...
    else if(convar == cvSkipBots)
        g_bSkipBots = view_as<bool>(StringToInt(newValue));
